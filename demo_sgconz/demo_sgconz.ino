 ...
 * continuosly stored to the statevars variable. Position estimates are also
 * calculated during intervals where there is no GPS data. The statevars are
 * continuously written to a file on the SD card. The program ends when the
 * start/stop button is pressed again.
 *
 * This application is a duplicate of the demo_sgcon_z app. It fixes the issue
 * of the oscillating GPS headings. Since a new statevars definition was created
 * while coming up with the fix, this new project was created. The robot file
 * (kintobor.c) was also modified during the fix.
 */
#include <stdint.h>
